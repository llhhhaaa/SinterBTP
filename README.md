...
## 📊 实验进展与论文初稿 (Experimental Progress)

系统经过四个阶段的严密实验验证（已完成全部闭环），证明了其在工业环境下的有效性与鲁棒性：

1.  **[实验 1：数据分析 (前期)](实验记录/1.Data%20Analysis%20(前期)/实验1_论文初稿.md)**
    -   **关键发现**：识别出烧结终点序列具有强反持久性（$H=0.364$）与明显的趋势平稳过程；确认 15# - 24# 风箱温度是 BTP 的核心物理驱动变量（互信息得分 > 0.5）。
2.  **[实验 2：统计显著性检验](实验记录/2.统计显著性检验/实验2_论文初稿.md)**
    -   **关键发现**：Enhanced Transformer 在 MAE 指标上较基准 Transformer 降低了 **35.40%**；Wilcoxon 符号秩检验显示 $p < 0.001$，证明性能提升具有极强的统计学意义。
3.  **[实验 3：消融实验](实验记录/3.消融实验/实验3_论文初稿.md)**
    -   **关键发现**：**RevIN** 是性能保持的关键，剔除后 MAE 激增 35.1%；**Fitting (物理拟合)** 模块显著提升了物理一致性，将原始传感器数据转化为高增益物理特征，对模型收敛速度贡献极大。
...
| 状态 | 英文 | 判定条件 | 生产建议 |
|------|------|----------|----------|
| **过烧** | Over-burn | 健康度低于阈值且 BTP 位置偏小 | 烧结终点提前到达，需提高机速或减少燃料 |
| **正常** | Normal | 健康度高于正常阈值 | 生产状态稳定，维持当前参数 |
| **欠烧** | Under-burn | 健康度低于阈值且 BTP 位置偏大 | 烧结终点滞后，需降低机速或增加燃料 |
...
-   **[Enhancement] 5×5 混淆矩阵热力图优化**：
    -   *Why*: 原热力图使用 Blues 颜色映射，颜色深度与数值关系不直观，难以凸显模型的分类准确率。
    -   *Solution*: 改用 YlOrRd 颜色映射（黄→橙→红），确保"数值越高颜色越深"；添加金色边框凸显对角线（正确分类）；添加颜色条增强可读性。
-   **[Enhancement] 交叉验证热力图优化**：
    -   *Why*: 原 RdYlGn 颜色映射对于越小越好的指标（MAE、MAPE、RMSE、Sharpness）存在语义混淆。
    -   *Solution*: 对越小越好的指标进行反转处理，确保统一的"颜色越绿越好"语义。
